...
# 구별, 동별 인구
df_pop = pd.read_excel('db_v0.7.xlsx', sheet_name='Database_2')
...
# 2. 인구대비 악취발생분석
# 구별 통계 산출
def get_pop_stat_gu(gu, start_date, end_date) : 
  try:
        global df_output
        ...
        for i in range(len(df_output)) :
            YM = df_output['연월'][i].split('-')[0] +'년'
            df_output['연도별 인구수'][i] = int(df_pop[df_pop['구'] == gu][df_pop['동'] == '소계'][YM])

        name = gu
        df_output[name] = ((df_output['민원 빈도수']/df_output['연도별 인구수'])*1000).astype(float)
...
# 구 1개 이상 선택시 통계 산출
def get_pop_stat_gus(gu_list, start_date, end_date, tot_num) : 
  global df_output
  get_pop_stat_gu_list = []
  for i in range(len(gu_list)) :
    get_pop_stat_gu(gu_list[i], start_date, end_date)
    get_pop_stat_gu_list.append(df_output)
    
  df_output = reduce(lambda left, right: pd.merge(left, right, on='연월', how = 'outer'), get_pop_stat_gu_list)
  df_output = df_output.sort_values(by = '연월').fillna(0)
  get_pop_stat_gus_sum(gu_list, df_output, tot_num) 
  write_to_html_file(df_output.set_index('연월').reset_index(),tot_num, 'templates/df{}.html'.format(tot_num))
  return print('생성')


# 구 1개 이상 선택시 그래프 산출
def get_pop_graph_gus(gu_list, start_date, end_date, tot_num) : 
  get_pop_stat_gu_list = []
  for i in range(len(gu_list)) :
    get_pop_stat_gu(gu_list[i], start_date, end_date)
    get_pop_stat_gu_list.append(df_output)
  if len(get_pop_stat_gu_list) > 1 :     
    #############################################
    trace_list = []
    for i in range(len(get_pop_stat_gu_list)) : 
      trace_list.append(go.Scatter(x = get_pop_stat_gu_list[i][get_pop_stat_gu_list[i].columns[0]], y = get_pop_stat_gu_list[i][get_pop_stat_gu_list[i].columns[1]],  name = gu_list[i]))
    fig = go.Figure(data = trace_list)
    fig.update_layout(title = {'text': "<구별 악취민원발생수>", 'x':0.5, 'xanchor': 'center', 'yanchor': 'top'}, xaxis_title="기간(연도/월)", yaxis_title="기간(연도/월)") 
    fig.update_layout(xaxis_tickfont_size=10, yaxis=dict(tickfont_size=10))
    fig.write_html('templates/graph_{}.html'.format(tot_num)) 
    #############################################
    df_merge_pre = get_pop_stat_gus(gu_list, start_date, end_date, tot_num)
  ...
  if len(get_pop_stat_gu_list) == 1 :     
    ...
    trace_list = []
    for i in range(len(get_pop_stat_gu_list)) : 
      trace_list.append(go.Bar(x = get_pop_stat_gu_list[i][get_pop_stat_gu_list[i].columns[0]], y = get_pop_stat_gu_list[i][get_pop_stat_gu_list[i].columns[1]],  name = gu_list[i]))
    fig = go.Figure(data = trace_list)
...
def get_map_by_gu(gu_list, start_date, end_date, tot_num) : 
  ...
  df_pop_gu_fin['1000명당 민원수'] = (df_pop_gu_fin['민원수']/df_pop_gu_fin[YM])*1000

  if '서울시' in gu_list:
    gu_list = list(df_dong_list.columns[1:])
  df_pop_gu_fin = df_pop_gu_fin[df_pop_gu_fin['구'].str.contains('|'.join(gu_list))].reset_index(drop=True)
  ...
  for i in range(len(data['features'])) : 
    ...
    for j in range(len(df_pop_gu_fin)) : 
      if data['features'][i]['properties']["자치구명"] == df_pop_gu_fin["구"][j] : 
        del data['features'][i]['properties']['code']
        del data['features'][i]['properties']['base_year']
        del data['features'][i]['properties']['name_eng']
        data['features'][i]['properties']["1000명당 민원수"] = round(df_pop_gu_fin["1000명당 민원수"][j],5)
        data['features'][i]['properties']["민원수(건)"] = int(df_output["민원수"][j])  
  ...
  viz = ChoroplethViz(access_token=token, data=data, color_property='1000명당 민원수', color_stops=color_stops,center = [126.986, 37.565], zoom=9.5, line_width=2)
  viz.create_html('templates/map_{}.html'.format(tot_num))
  return print('생성')


# 동별 통계 산출
def get_pop_stat_dong(gu, dong, start_date, end_date) : 
    try:
          global df_output
          df_fin = df[(df['datetime'] >= start_date)&(df['datetime'] <= dt.datetime.strptime(end_date, '%Y-%m') + relativedelta(months=1))]
          ...
          for i in range(len(df_output)) :
            YM = df_output['연월'][i].split('-')[0] +'년'
            df_output['연도별 인구수'][i] = df_pop[(df_pop['구'] == gu)&(df_pop['동'] == dong)][YM]
          name = dong
          df_output[name] = ''
...
# 동 1개 이상 선택시 통계 산출
def get_pop_stat_dongs(gu, dong_list, start_date, end_date, tot_num) : 
  global df_output
  get_pop_stat_dong_list = []
  if '전체' in dong_list :
      dong_list = seoul_dict[gu].pop('전체')

  for i in range(len(dong_list)) :
    get_pop_stat_dong(gu, dong_list[i], start_date, end_date)
    get_pop_stat_dong_list.append(df_output)

  df_output = reduce(lambda left, right: pd.merge(left, right, on='연월', how = 'outer'), get_pop_stat_dong_list)
  df_output = df_output.sort_values(by = '연월').fillna(0)
  get_pop_stat_gus_sum(dong_list, df_output, tot_num)
  write_to_html_file(df_output.set_index('연월').reset_index(), tot_num, 'templates/df{}.html'.format(tot_num))
  return print('생성')
...
# 동 1개 이상 선택시 그래프
def get_pop_graph_dongs(gu, dong_list, start_date, end_date, tot_num) : 
  get_pop_stat_dong_list = []
  if '전체' in dong_list :
      dong_list = seoul_dict[gu].pop('전체')
  for i in range(len(dong_list)) :
  ...
  if len(get_pop_stat_dong_list) > 1 :     
    #############################################  
    trace_list = []
    for i in range(len(get_pop_stat_dong_list)) :
      trace_list.append(go.Scatter(x = get_pop_stat_dong_list[i][get_pop_stat_dong_list[i].columns[0]], y = get_pop_stat_dong_list[i][get_pop_stat_dong_list[i].columns[1]],  name = dong_list[i]))
    fig = go.Figure(data = trace_list)
    fig.update_layout(xaxis_tickfont_size=10, yaxis=dict(tickfont_size=10))
    fig.update_layout(title = {'text': "<동별 악취민원발생수>", 'x':0.5, 'xanchor': 'center', 'yanchor': 'top'}, xaxis_title="기간(연도/월)", yaxis_title="악취민원 건수(연/월)")
    fig.write_html('templates/graph_{}.html'.format(tot_num))
    #############################################
    df_merge_pre = get_pop_stat_dongs(gu, dong_list, start_date, end_date, tot_num)
    df_merge = df_output.sort_values(by = '연월').fillna(0).reset_index(drop=True)
    ax = df_merge.plot(x ='연월', figsize = (18,12))
    ax.set_xlabel('기간(연도/월)')
  ...
  if len(get_pop_stat_dong_list) == 1 :     
    ...
    trace_list = []
    for i in range(len(get_pop_stat_dong_list)) :
      trace_list.append(go.Bar(x = get_pop_stat_dong_list[i][get_pop_stat_dong_list[i].columns[0]], y = get_pop_stat_dong_list[i][get_pop_stat_dong_list[i].columns[1]],  name = dong_list[i]))
    fig = go.Figure(data = trace_list)
    fig.update_traces(text = trace_list[0]['y'],  textposition='outside')
    fig.update_traces(marker_color='rgb(0,0,0)')
    fig.update_layout(xaxis_tickfont_size=10, yaxis=dict(tickfont_size=10))
...
# 동별 지도
def get_map_by_dong(gu, dong_list, start_date, end_date, tot_num) : 
  ...
  df_pop_dong = df_pop[df_pop['구'] == gu][['동',YM]].reset_index(drop=True)
  df_pop_dong_fin = pd.merge(df_output, df_pop_dong, on = '동', how='left' )
  df_pop_dong_fin['1000명당 민원수'] = (df_pop_dong_fin['민원수']/df_pop_dong_fin[YM])*1000
  ...
  if '전체' in dong_list :
    ...
    df_pop_dong_fin = df_pop_dong_fin[df_pop_dong_fin['동'].str.contains('|'.join(dong_list))].reset_index(drop=True)
  else :
    df_pop_dong_fin = df_pop_dong_fin[df_pop_dong_fin['동'].str.contains('|'.join(dong_list))].reset_index(drop=True)
  ...
  for i in range(len(data['features'])) : 
    ...
    for j in range(len(df_pop_dong_fin)) : 
      if data['features'][i]['properties']['자치구명'] == gu and data['features'][i]['properties']["동명"] == df_pop_dong_fin["동"][j] : 
            ...
            data['features'][i]['properties']["1,000명당 민원수"] = round(df_pop_dong_fin["1000명당 민원수"][j], 5)
            data['features'][i]['properties']["분석기간"] =  start_date + "~" + end_date
            data_list.append(data['features'][i])

  data['features'] = data_list  
  token = 'pk.eyJ1IjoiZHVjazk2NjciLCJhIjoiY2thNzRiMXhxMGQ5NTJ0cXB0dGpmZ3RrOSJ9.f45On_lrv6Nm4iO7oCg7nw'
  res, bins = pd.qcut(df_pop_dong_fin['1000명당 민원수'], 3, retbins=True)
  color_breaks = list(bins) 
  color_stops = create_color_stops(color_breaks, colors='BuPu')

  viz = ChoroplethViz(access_token=token, data=data, color_property='1,000명당 민원수', color_stops=color_stops, center = [126.986, 37.565],zoom=9.5, line_width=2)
  viz.create_html('templates/map_{}.html'.format(tot_num))
  return print('생성')


#평균, 최대 최소 출력용
def get_pop_stat_gus_sum(gu_list, df_output, tot_num):
    print(df_output)
...
def get_pop_export( gu_list, gu , dong_list , start_date, end_date, tot_num) :
    if gu_list:
        if gu_list == ['서울시'] :
            gu_list_temp = list(df_dong_list.columns[1:])
            get_pop_graph_gus(gu_list_temp, start_date, end_date, tot_num)
            get_map_by_gu(gu_list,start_date, end_date, tot_num)

        elif '서울시' not in gu_list:
            get_pop_graph_gus(gu_list, start_date, end_date, tot_num)
            get_map_by_gu(gu_list,start_date, end_date, tot_num)
    else:
        if  (dong_list[0] == "전체") :
            dong_list_temp = list(df_dong_list[gu].dropna())

            get_pop_stat_dongs(gu, dong_list_temp, start_date, end_date, tot_num)
            get_pop_graph_dongs(gu, dong_list_temp, start_date, end_date, tot_num)
            get_map_by_dong(gu, dong_list, start_date, end_date, tot_num)

        elif (dong_list != ['전체']) :
            get_pop_stat_dongs(gu, dong_list, start_date, end_date, tot_num)
            get_pop_graph_dongs(gu, dong_list, start_date, end_date, tot_num)
            get_map_by_dong(gu, dong_list, start_date, end_date, tot_num)
